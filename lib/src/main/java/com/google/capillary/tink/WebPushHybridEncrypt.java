...
package com.google.capillary.tink;
 ...
 * <pre>
 * +-----------+----------------+------------------+--------------------------------
 * | salt (16) | recordsize (4) | pk-len (1) | pk (pk-len) | aes128-gcm-ciphertext |
 * +-----------+----------------+------------------+--------------------------------
 * </pre>
 *
...
public final class WebPushHybridEncrypt implements HybridEncrypt {
  ...
  public byte[] encrypt(final byte[] plaintext, final byte[] contextInfo /* unused */)
    ...
    byte[] ikm =
        WebPushUtil
            .computeIkm(ecdhSecret, authSecret, recipientPublicKey, ephemeralPublicKeyBytes);
    ...
    return ByteBuffer.allocate(
        WebPushConstants.CIPHERTEXT_OVERHEAD + plaintext.length)
        .put(salt)
        .putInt(recordSize)
